...
- **多种练习**：词汇测验（支持 A–D / 1–4 键盘作答）、抽认卡学习、造句、听力听写、拼写填空，以及 **跟读**（基于浏览器语音识别给发音打分，从“会写”走向“会说”）。
- **学习分析**：本月学习热力图、掌握率趋势折线图、全局单词状态点图。
- **成就系统**：掌握里程碑与连续学习天数等 11 项成就。
- **今日学习（引导式）**：一键把「到期复习 + 新词」串成一条流程，新词按**高频优先**(基础/进阶/高阶分级)出现，降低决策摩擦、保证每日间隔复习。
- **每日目标 + 连续天数**：进度环显示当日已学 / 目标，并展示连续学习天数。
